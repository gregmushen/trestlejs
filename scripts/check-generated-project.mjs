...
try {
  ...
  if (process.env.TRESTLE_GENERATED_DATABASE_URL) {
    ...
    await run("pnpm", ["--filter", "./packages/db", "exec", "vitest", "run", "src/inbox.integration.test.ts", "src/outbox-retention.integration.test.ts"], project, { TRESTLE_INBOX_TEST_DATABASE_URL: process.env.TRESTLE_GENERATED_DATABASE_URL, TRESTLE_INBOX_TEST_ADMIN_DATABASE_URL: process.env.TRESTLE_GENERATED_DATABASE_URL });
