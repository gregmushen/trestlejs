...
describe("versioned project upgrades", () => {
  it("plans without mutation and applies convergently while preserving custom guidance", async () => {
    ...
    await writeFile(path.join(root, "packages/db/src/auth-schema.ts"), "roleSource: text('role_source'),\n");
    await writeFile(path.join(root, "packages/db/migrations/0009_access_control.sql"), 'CREATE TABLE "application_role_assignment" ("id" uuid);\n');
