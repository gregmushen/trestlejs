...
export async function validateCi(root: string): Promise<CiValidationReport> {
  ...
  checks.push(check("ci.preview.provider-preflight", preview.includes("cloudflare-preflight.mjs") && /^\s+node scripts\/neon-preflight\.mjs\s*$/mu.test(preview) && occursInOrder(preview, "Verify Neon project access before provisioning", "Provision isolated Neon branch"), "preview verifies Cloudflare and Neon access before provisioning resources"));
