/**
 * Authentication assurance (docs/INTEGRATION_STRATEGY.md §3.2). Better Auth
 * owns the credential protocols; Trestle records, per session, how the
 * session was authenticated and when, and sensitive actions require a level
 * and freshness from that evidence rather than from session age or from
 * `twoFactorEnabled` on the account.
 */
...
const phishingResistantPermissions: ReadonlySet<string> = new Set(["platform.roles.manage", "platform.support.enter_tenant", "platform.access_catalog.manage", "platform.auth_policy.manage"]);
 ...
 */
export function platformAssuranceRequirement(permission: string, environment: "local" | "preview" | "staging" | "production"): AssuranceRequirement {
