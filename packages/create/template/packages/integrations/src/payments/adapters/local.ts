...
export class LocalBillingAdapter implements BillingService {
  constructor(private readonly repository: BillingProjectionRepository, private readonly plans: Record<string, readonly string[]>) {}
  ...
  async changePlan(input: ChangePlanInput) { const current = await this.required(input.organizationId); await this.repository.put({ ...current, plan: input.plan, entitlements: [...this.entitlements(input.plan)] }); }
  async activate(input: { organizationId: string; plan: string }) { await this.repository.put({ organizationId: input.organizationId, provider: "local", plan: input.plan, status: "active", cancelAtPeriodEnd: false, entitlements: [...this.entitlements(input.plan)] }); }
