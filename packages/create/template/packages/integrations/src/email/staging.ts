...
export class StagingRedirectEmailService implements EmailService {
  constructor(private readonly inner: EmailService, private readonly recipient: EmailAddress, private readonly environment: "preview" | "staging" = "staging") {}
  ...
  private redirect(message: EmailMessage): EmailMessage {
    ...
    return { ...message, to: this.recipient, cc: [], bcc: [], subject: `[${this.environment.toUpperCase()} → ${original}] ${message.subject}` };
  }
}
