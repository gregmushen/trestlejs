...
import { CloudflareR2ArtifactStore, LocalArtifactStore, createArtifactSigner } from "./index.js";
...
describe("tenant-owned artifact storage", () => {
  ...
  it("prefixes R2 keys with tenant ownership and preserves metadata", async () => { const calls: unknown[] = []; const bytes = new Uint8Array([4, 5]); const bucket = { put: async (...args: unknown[]) => { calls.push(args); }, get: async () => ({ size: 2, arrayBuffer: async () => bytes.buffer }), delete: async (key: string) => { calls.push(key); } }; const store = new CloudflareR2ArtifactStore(bucket); await store.put({ id: "a", organizationId: "org-a", key: "x.bin", contentType: "application/octet-stream", body: bytes }); expect(calls[0]).toEqual(["org-a/x.bin", bytes, expect.objectContaining({ customMetadata: { artifactId: "a", organizationId: "org-a" } })]); expect((await store.get("org-a", "a"))?.body).toEqual(bytes); expect(await store.get("org-b", "a")).toBeNull(); });
