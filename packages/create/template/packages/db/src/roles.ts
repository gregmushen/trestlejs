...
export async function verifyRuntimeRoleDataAccess(connectionString: string): Promise<void> {
  const sql = postgres(connectionString, { max: 1, prepare: false });
  try {
    const [access] = await sql<{ auth_read: boolean; receipt_write: boolean; tenant_read: boolean; webhook_read: boolean }[]>`
    ...
    if (access.tenant_read || access.webhook_read) throw new Error("Runtime login can read tenant records without assuming the RLS role");
