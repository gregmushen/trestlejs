...
export class PostgresOutboxStore implements OutboxStore {
  ...
  async lease(limit = 10, leaseMs = 30_000): Promise<OutboxEntry[]> {
    ...
    for (const row of rows) {
      // One malformed message must not block the batch: it goes straight to the dead-letter state.
      try { entries.push(entry(row)); } catch { await this.sql`update outbox_message set status='dead', leased_until=null, last_error='invalid event envelope' where id=${row.id}`; }
    }
    return entries;
  }
...
/** Outbox insert for use inside a tenant repository's atomic batch (see ctx.events.statement). */
export function outboxStatement(message: EventEnvelope): SQL {
  const parsed = eventEnvelopeSchema.parse(message);
  return sql`insert into outbox_message (id, event_name, schema_version, occurred_at, resource_type, resource_id, correlation_id, causation_id, idempotency_key, payload, available_at)
    values (${parsed.id}, ${parsed.name}, ${parsed.schemaVersion}, ${new Date(parsed.occurredAt)}, ${parsed.resource.type}, ${parsed.resource.id}, ${parsed.correlationId}, ${parsed.causationId ?? null}, ${parsed.idempotencyKey}, ${JSON.stringify(parsed.payload)}::text::jsonb, ${new Date(parsed.occurredAt)})
