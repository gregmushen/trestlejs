...
export class PostgresArtifactMetadataRepository implements ArtifactMetadataRepository {
  ...
  async put(metadata: ArtifactMetadata): Promise<ArtifactMetadata> {
    const [record] = await this.database.insert(artifactMetadata).values({ id: metadata.id, organizationId: metadata.organizationId, storageKey: metadata.key, contentType: metadata.contentType, size: metadata.size, createdAt: metadata.createdAt }).onConflictDoNothing().returning();
  ...
  async discard(organizationId: string, id: string, key: string): Promise<boolean> {
    return (await this.database.delete(artifactMetadata).where(and(eq(artifactMetadata.id, id), eq(artifactMetadata.organizationId, organizationId), eq(artifactMetadata.storageKey, key), isNull(artifactMetadata.deletedAt))).returning()).length > 0;
  }
}
