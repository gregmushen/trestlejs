...
export function computeCapabilityStatus(declared: DeclaredCapabilities, environment: RuntimeEnvironment, env: Readonly<Record<string, unknown>>): CapabilityStatus[] {
  ...
  return [
    ...
    status("queues", declared.queues, missing("TRESTLE_EVENTS")),
    status("workflows", declared.workflows, missing("TRESTLE_WORKFLOW")),
    status("r2", declared.r2, environment === "local" ? [] : missing("TRESTLE_ARTIFACTS")),
