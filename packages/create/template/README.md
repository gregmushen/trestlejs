...
New resources generated by `trestle generate resource` register a private
`resource.<name>.created` application event and use this transactional path
for create operations. They do not automatically expose that event as a
customer webhook or emit update/delete events; add those deliberately with
appropriate idempotency and public projection rules.
