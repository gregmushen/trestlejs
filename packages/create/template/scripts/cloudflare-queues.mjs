...
export function queueClient({ accountId, token, fetcher = fetch }) {
  ...
  async function verify(name) {
    const matches = (await list()).filter((queue) => queue.queue_name === name);
    if (matches.length !== 1 || !/^[a-f0-9]{32}$/u.test(matches[0]?.queue_id ?? "")) {
      throw new Error(`Cloudflare Queue ${name} is missing or has an invalid identity`);
    }
