...
async function call(method: string, path: string) {
  const response = await admin.request(path, { method, headers: { origin: "http://localhost:42070" } }, environment);
...
describe("platform admin Worker", () => {
  ...
  it("declares a policy for every route, a route for every policy, and a policy for every view", () => {
    ...
    for (const view of adminViews) for (const route of view.api) {
      expect(adminRoutePolicies.find((policy) => policy.method === route.method && policy.path === route.path)?.permission).toBe(view.permission);
    }
  });
});
