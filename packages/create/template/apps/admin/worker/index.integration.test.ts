...
suite("platform admin Worker against PostgreSQL", () => {
  ...
  it("signs a real account in on the admin origin and requires a platform role", async () => {
    ...
    // A fresh password starts the first enrollment; once a factor exists, factor changes need a second factor.
    ...
    await expect((await enable()).json()).resolves.toMatchObject({ error: "step_up_required", required: "mfa" });
