...
describe("admin view registry", () => {
  ...
  it("filters navigation by platform permission and capability state without being a security boundary", () => {
    const registry = buildAdminRegistry([view(), view({ id: "keys", path: "/keys", permission: "platform.machine_access.read", capability: "apiKeys" }), view({ id: "mail", path: "/mail", permission: "platform.email.read", capability: "email" })], options);
    ...
    expect(items(["platform.organizations.read", "platform.machine_access.read", "platform.email.read"], { apiKeys: { state: "deployed" }, email: { state: "declared", message: "Email delivery is not configured for staging." } }))
      .toEqual([["contracts", "enabled"], ["keys", "enabled"], ["mail", "unconfigured"]]);
    const unconfigured = visibleNavigation(registry, { permissions: new Set(["platform.email.read"]), capabilityStates: { email: { state: "declared" } }, environment: "staging" }).groups[0]!.items[0]!.availability;
    ...
    expect(items(["platform.email.read"], { email: { state: "disabled" } })).toEqual([]);
...
describe("commands and hotkeys", () => {
  ...
  it("composes commands from view metadata with kind, scope, and permission defaults", () => {
    const registry = buildAdminRegistry([withCommands("contracts", "/contracts", [
      ...
      { id: "contracts.revoke", label: "Revoke a contract", hotkey: "r", kind: "action", scope: "selection", requires: "a contract", destructive: true, permission: "platform.machine_access.revoke", keywords: ["cancel"] },
    ])], options);
    expect(registry.commands).toEqual([
      ...
      { id: "contracts.revoke", label: "Revoke a contract", hotkey: "r", permission: "platform.machine_access.revoke", keywords: ["cancel"], kind: "action", scope: "selection", destructive: true, requires: "a contract", viewId: "contracts", path: "/contracts", group: "Customers", icon },
    ]);
  });
  ...
  it("offers only commands the operator may run and searches labels and keywords", () => {
    const registry = buildAdminRegistry([
      withCommands("keys", "/keys", [{ id: "keys.open", label: "Go to API keys", hotkey: "g k" }, { id: "keys.revoke", label: "Revoke an API key", permission: "platform.machine_access.revoke", kind: "action", scope: "selection", requires: "a key", destructive: true, keywords: ["leaked"] }], "platform.machine_access.read"),
      withCommands("users", "/users", [{ id: "users.open", label: "Go to users" }, { id: "users.revoke-sessions", label: "Revoke user sessions", permission: "platform.sessions.revoke", kind: "action", scope: "selection", requires: "a user", destructive: true }], "platform.users.read"),
    ], options);
    ...
    expect(availableCommands(registry, context(["platform.users.read"])).map((command) => command.id)).toEqual(["users.open"]);
    const all = availableCommands(registry, context(["platform.machine_access.read", "platform.machine_access.revoke", "platform.users.read", "platform.sessions.revoke"]));
