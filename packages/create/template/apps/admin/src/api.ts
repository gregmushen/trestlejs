...
import type { EffectiveEntitlement, Feature, PlanVersion, PlanVersionState, PrivilegeValue, QuotaState, ReconciliationResult } from "./billing-model";
import { z } from "zod";
...
// import.meta.env is absent outside Vite (the view check runs under tsx).
...
export type WebhookEndpointSummary = {
  ...
  /** Delivery health reported by the webhook pipeline. */
  health?: string;
};
...
export const api: AdminApi = createAdminApi({ baseUrl: adminApiOrigin });
