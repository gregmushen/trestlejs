...
export function mainBackend(request: Request, reasoned: (reason: string) => { reason: string }) {
  ...
  return {
    ...
    overview: async (): Promise<Overview> => {
      ...
      for (const status of caps.capabilities.filter((item) => item.state === "declared")) exceptions.push({ kind: `capability:${status.id}`, severity: "warning", title: `${status.label} is not configured`, detail: status.repair ? `Run ${status.repair}` : "Run pnpm exec trestle doctor to see how to configure it.", href: "/system/health" });
