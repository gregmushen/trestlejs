...
function KeyDrawer(props: { id: string; open: boolean; onClose: () => void; confirm: ReturnType<typeof useConfirmAction>; onChanged: () => void; onIssued: (issued: IssuedKey) => void }) {
  ...
  return <AdminDetailDrawer open={props.open} onClose={props.onClose} width="lg" title={key?.name ?? (key ? maskedKeyPrefix(key.displayPrefix) : "API key")} subtitle={key ? <AdminCode>{maskedKeyPrefix(key.displayPrefix)}</AdminCode> : undefined}
    actions={key && usable && manage ? <>
      <Button variant="secondary" onClick={rotate}>Rotate</Button>
      <Button variant="secondary" onClick={() => setScopes(scopes === null ? [...key.scopes] : null)}>{scopes === null ? "Change scopes" : "Cancel scope change"}</Button>
      {can("platform.machine_access.revoke") && <Button variant="secondary-destructive" onClick={() => props.confirm.open({ title: "Revoke API key", confirmLabel: "Revoke key", destructive: true, scope: [`${maskedKeyPrefix(key.displayPrefix)} fails closed immediately`, "the key remains as a historical record"], onConfirm: (reason) => api.revokeApiKey(key.id, reason), onDone: changed })}>Revoke</Button>}
    </> : undefined}>
...
export default function ApiKeysView() {
  ...
  const canRevoke = can("platform.machine_access.revoke");
  const manage = can("platform.machine_access.manage");
  ...
  useAdminCommands({
    "api-keys.new": { enabled: manage, run: () => setCreating(true) },
  ...
  return <>
    <AdminPageHeader title="API keys" description="Safe metadata only. Nobody can recover a key's secret: compromised keys are revoked, and scope changes issue a replacement."
      actions={manage ? <Button variant="primary" icon={<PlusIcon />} onClick={() => setCreating(true)}>Create API key</Button> : undefined} />
    ...
    <CreateApiKeyDialog open={creating} onClose={() => setCreating(false)} {...(organizationId ? { initial: { organizationId } } : {})} confirm={confirm} onIssued={(result) => { setIssued(result); refresh(); }} />
