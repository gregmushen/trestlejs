...
export default function SupportSessionsView() {
  ...
  const canRevoke = can("platform.support.revoke");
  const start = () => void navigate({ to: "/organizations" as never, search: { command: "organizations.start-support-session" } as never });
  useAdminCommands({
    "support-sessions.start": { enabled: can("platform.support.enter_tenant"), run: start },
    "support-sessions.exit": { enabled: Boolean(supportSession), run: () => void exitSupportSession() },
    "support-sessions.revoke": { enabled: Boolean(selected && canRevoke && stateOf(selected) === "active"), ...(selected ? { target: selected.id } : {}), confirm: () => { if (selected) confirm.open(revoke(selected)); } },
  });
  return <>
    <AdminPageHeader title="Support sessions" description="Every tenant-context support session: who, where, why, which profile, and what they did. Security administrators can revoke an active session."
      ...
      actions={<>{supportSession && <Button variant="secondary" onClick={() => void exitSupportSession()}>Exit your session</Button>}{can("platform.support.enter_tenant") && <Button variant="primary" onClick={start}>Start a session</Button>}</>} />
