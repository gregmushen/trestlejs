...
export default function AuditView() {
  ...
  useAdminCommands({
    ...
    "audit.open-support-session": { enabled: Boolean(selected?.supportSessionId) && can("platform.support.read"), ...(selected ? { target: selected.id } : {}), run: () => { if (selected?.supportSessionId) openSession(selected.supportSessionId); } },
  });
  ...
  return <>
    ...
    <AdminQueryState query={events} isEmpty={(data) => data.events.length === 0} empty="No audit events match.">{(data) => <>
      ...
      <AdminDataTable caption="Audit events" selectable rows={data.events} rowKey={(row) => row.id} rowLabel={(row) => row.name} server={{ totalCount: data.total, pageSize: data.pageSize }}
        rowActions={(row) => [
          ...
          ...(row.supportSessionId && can("platform.support.read") ? [{ label: "Open support session", hotkey: "s", run: () => openSession(row.supportSessionId!) }] : []),
        ]}
        columns={[
    ...
    <AdminDetailDrawer open={Boolean(search.selected)} onClose={() => update({ selected: undefined })} title={selected?.name ?? "Audit event"} subtitle={selected ? formatDate(selected.occurredAt) : undefined}
      actions={selected ? <>
        ...
        {selected.supportSessionId && can("platform.support.read") && <Button variant="secondary" onClick={() => openSession(selected.supportSessionId!)}>Support session</Button>}
      </> : undefined}>
