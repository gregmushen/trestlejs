...
export default function ServiceAccountsView() {
  ...
  useAdminCommands({
    "service-accounts.new": { enabled: manage, run: () => setCreating({ organizationId: organizationId ?? "", name: "", description: "", roles: [] }) },
    "service-accounts.suspend": { enabled: Boolean(detail.row && can("platform.machine_access.revoke") && detail.row.status === "active"), ...(detail.row ? { target: detail.row.id } : {}), confirm: () => { if (detail.row) confirm.open(suspend(detail.row)); } },
  });
  const submit = async () => {
