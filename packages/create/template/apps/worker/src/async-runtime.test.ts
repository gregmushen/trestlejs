...
describe("Worker Queue consumer", () => {
  it("validates, dispatches, and acknowledges a registered versioned event", async () => { const handled: string[] = []; const registry = new EventConsumerRegistry<{ marker: string }>(); registry.register({ name: "article.published", schemaVersion: 1, parse: (payload) => payload as { title: string } }, async (payload, _event, environment) => { handled.push(`${environment.marker}:${payload.title}`); }); const states: string[] = []; const result = await createQueueConsumer(registry)({ messages: [{ body: envelope(), ack: () => states.push("ack"), retry: () => states.push("retry") }] }, { marker: "worker" }); expect(result).toEqual({ acknowledged: 1, retried: 0 }); expect(states).toEqual(["ack"]); expect(handled).toEqual(["worker:Hello"]); });
