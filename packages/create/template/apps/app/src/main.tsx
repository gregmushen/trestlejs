...
function BillingSettings() {
  const subscription = useQuery({ queryKey: ["billing-subscription"], queryFn: async () => { const response = await fetch(api("/api/billing/subscription"), { credentials: "include" }); if (!response.ok) throw new Error("Select an organization before managing billing"); return response.json() as Promise<{ subscription: null | { plan: string; status: string; currentPeriodEnd?: string; cancelAtPeriodEnd: boolean; entitlements: string[] } }>; } });
  const [message, setMessage] = useState<string>();
  ...
  return <section className="card p-8"><p className="eyebrow">Settings</p><h1 className="mt-2 text-3xl font-semibold">Billing</h1>{subscription.isPending ? <p className="mt-4">Loading…</p> : subscription.error ? <p className="mt-4 text-red-700">{subscription.error.message}</p> : subscription.data?.subscription ? <div className="mt-6 space-y-2"><p>Current plan: <strong>{subscription.data.subscription.plan}</strong></p><p>Status: {subscription.data.subscription.status}</p><p>{subscription.data.subscription.cancelAtPeriodEnd ? "Cancels at period end" : "Renews automatically"}</p><button className="button mt-4" onClick={() => void manage()}>Manage billing</button></div> : <p className="mt-4">No active subscription.</p>}{message && <p className="mt-3 text-red-700">{message}</p>}</section>;
}
